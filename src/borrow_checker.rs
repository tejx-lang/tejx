...
impl BorrowChecker {
    ...
    pub fn check(&mut self, func: &MIRFunction, filename: &str) -> HashMap<usize, Vec<String>> {
    ...
    fn check_use(&mut self, val: &MIRValue, state: &HashMap<String, VarState>, report: bool, line: usize) {
        if let MIRValue::Variable { name, .. } = val {
             ...
             if let Some(s) = state.get(name) {
                 match s {
                     VarState::Moved => {
                         if report { self.errors.push(Diagnostic::new(format!("Use of moved variable '{}'", name), line, 1, self.filename.clone())); }
                     },
                     VarState::MaybeMoved => {
                          if report { self.errors.push(Diagnostic::new(format!("Use of possibly moved variable '{}'", name), line, 1, self.filename.clone())); }
                     },
                     VarState::Uninitialized => {
                          // if report { self.errors.push(Diagnostic::new(format!("Use of uninitialized variable '{}'", name), line, 1, self.filename.clone())); }
                     },
                     _ => {}
                 }
             }
