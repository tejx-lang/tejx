...
impl Parser {
    ...
    pub fn get_errors(&self) -> &Vec<crate::common::diagnostics::Diagnostic> {
        &self.errors
    }
    ...
    fn parse_function_declaration(&mut self, is_async: bool, is_extern: bool) -> Statement {
        ...
        if self.match_token(TokenType::Less) {
            loop {
                ...
                generic_params.push(crate::frontend::ast::GenericParam {
                    name: param_name,
                    bound,
                });
    ...
    fn parse_class_declaration(&mut self, is_abstract: bool) -> Statement {
        ...
        if self.match_token(TokenType::Implements) {
            loop {
                implemented_protocols.push(
                    self.consume(TokenType::Identifier, "Expected protocol name")
                        .value
                        .clone(),
                );
                if !self.match_token(TokenType::Comma) {
            ...
            if self.check(TokenType::Less) {
                ...
                if is_generic {
                    ...
                    loop {
                        ...
                        method_generic_params.push(crate::frontend::ast::GenericParam {
                            name: param_name,
                            bound,
                        });
                        if !self.match_token(TokenType::Comma) {
    ...
    fn parse_type_alias_declaration(&mut self) -> Statement {
        let start = self
            .consume(TokenType::TypeAlias, "Expected 'type'")
            .clone();
    ...
    fn parse_enum_declaration(&mut self) -> Statement {
        ...
        while !self.check(TokenType::CloseBrace) && !self.is_at_end() {
            let member_name = self
                .consume_identifier("Expected enum member")
                .value
                .clone();
            let mut value = None;
            if self.match_token(TokenType::Equals) {
    ...
    fn parse_import_statement(&mut self) -> Statement {
        ...
        if self.check(TokenType::Identifier) && self.peek().value == "std" {
            ...
            let source = self
                .consume(TokenType::String, "Expected module path")
                .value
                .clone();
            self.consume(TokenType::Semicolon, "Expected ';'");
        ...
        if self.check(TokenType::Identifier) {
            ...
            if self.check(TokenType::Identifier) && self.peek().value == "as" {
                ...
                alias = Some(
                    self.consume_identifier("Expected alias after 'as'")
                        .value
                        .clone(),
                );
            }
    ...
    fn parse_interface_declaration(&mut self) -> Statement {
        let start = self
            ...
            .clone();
        let name = self
            .consume_identifier("Expected interface name")
            .value
            .clone();
        self.consume(TokenType::OpenBrace, "Expected '{'");
    ...
    fn parse_type_annotation(&mut self) -> TypeNode {
        ...
        while self.match_token(TokenType::Pipe) {
            ...
            if union_types.is_empty() {
                union_types.push(base_type.clone());
            }
    ...
    fn parse_single_type(&mut self) -> TypeNode {
        self.parse_base_type()
    }

    fn parse_base_type(&mut self) -> TypeNode {
        ...
        if self.match_token(TokenType::OpenBrace) {
            let mut member_nodes: Vec<(String, bool, TypeNode)> = Vec::new();
            while !self.check(TokenType::CloseBrace) && !self.is_at_end() {
                let key = self
                    .consume_identifier("Expected property name")
                    .value
        ...
        if self.check(TokenType::Identifier) || self.is_keyword_identifier() {
            ...
            if self.match_token(TokenType::Less) {
                ...
                if self.check(TokenType::Greater) {
                    ...
                    self.tokens[self.current] = crate::frontend::token::Token {
    ...
    fn parse_try_statement(&mut self) -> Statement {
        ...
        let mut catch_block = Box::new(Statement::BlockStmt {
            statements: vec![],
            _line: 0,
            _col: 0,
        }); // Empty by default
    ...
    fn parse_comma_expression(&mut self) -> Expression {
        ...
        if self.check(TokenType::Comma) {
            ...
            return Expression::SequenceExpr {
                expressions,
                _line: line,
                _col: col,
            };
    ...
    fn parse_assignment(&mut self) -> Expression {
        ...
        if self.match_token(TokenType::Equals)
            || self.match_token(TokenType::PlusEquals)
            || self.match_token(TokenType::MinusEquals)
            || self.match_token(TokenType::StarEquals)
            || self.match_token(TokenType::SlashEquals)
            || self.match_token(TokenType::ModuloEquals)
            || self.match_token(TokenType::AmpersandEquals)
            || self.match_token(TokenType::PipeEquals)
            || self.match_token(TokenType::CaretEquals)
            || self.match_token(TokenType::LessLessEquals)
    ...
    fn parse_comparison(&mut self) -> Expression {
        ...
        while self.match_token(TokenType::GreaterEqual)
    ...
    fn is_generic_call_lookahead(&self) -> bool {
        ...
        if self.tokens[i].token_type != TokenType::Less {
            return false;
        }
        i += 1;
        let mut depth = 1;
        while i < self.tokens.len() && self.tokens[i].token_type != TokenType::EndOfFile {
    ...
    fn parse_call(&mut self) -> Expression {
        ...
        loop {
            ...
            if self.is_generic_call_lookahead() {
                ...
                if !self.check(TokenType::Greater) {
                    loop {
                        t_args.push(self.parse_type_annotation());
                        if !self.match_token(TokenType::Comma) {
                            break;
                        }
                    }
            ...
            if self.match_token(TokenType::OpenParen) {
                ...
                let name = self
                    .consume_identifier("Expected property name after separator")
                    .value
                    .clone();
                let _line = self.tokens[self.current - 1].line;
                let _col = self.tokens[self.current - 1].column;
        ...
        match token.token_type {
            ...
            _ => {
                ...
                Expression::Identifier {
                    ...
                    _line: err_token.line,
                    _col: err_token.column,
                }
            }
        }
    }
    ...
    fn parse_template_string(&mut self, token: Token) -> Expression {
        ...
        while let Some(start_idx) = value[current_pos..].find("${") {
            ...
            if let Some(actual_end) = end_idx {
                ...
                // Parse interpolation
                let mut lexer = crate::frontend::lexer::Lexer::new(inter_content, &self.filename);
                let tokens = lexer.tokenize();
                let mut parser = crate::frontend::parser::Parser::new(tokens, &self.filename);
        ...
        expr.unwrap_or(Expression::StringLiteral {
            value: "".to_string(),
            _line: token.line,
            _col: token.column,
        })
