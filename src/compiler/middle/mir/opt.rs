                ...
                match inst {
                    MIRInstruction::Move {
                        ...
                        if aliases.contains(name) {
                            ...
                            if matches!(
                                dst_ty,
                                Some(TejxType::DynamicArray(_)) | Some(TejxType::FixedArray(_, _))
                            ) {
                                continue;
                            }
                            return None;
    ...
    fn rewrite_local_string_appends(&self, func: &mut MIRFunction) -> bool {
        ...
        for block in &mut func.blocks {
            ...
            while i < block.instructions.len() {
                ...
                {
                    if value.is_empty() && candidates.contains(dst) && assigned_so_far.contains(dst)
                    {
        ...
        for block in &func.blocks {
            for inst in &block.instructions {
                match inst {
                    ...
                    MIRInstruction::Return {
                        value: Some(MIRValue::Variable { name: ret_name, .. }),
                        ..
                    } if ret_name == name => {
                        return false;
                    }
                    MIRInstruction::Call { callee, args, .. } => {
                        for arg in args {
                            if let MIRValue::Variable { name: arg_name, .. } = arg {
                                if arg_name == name && callee != "rt_len" && callee != "rt_strlen" {
                                    return false;
                                }
                            }
                        }
                    ...
                    MIRInstruction::LoadMember {
                        obj: MIRValue::Variable { name: obj_name, .. },
                        ..
                    }
                    | MIRInstruction::LoadIndex {
                        obj: MIRValue::Variable { name: obj_name, .. },
    ...
    fn fold_constants(&self, func: &mut MIRFunction) -> bool {
        ...
        for block in &mut func.blocks {
            for i in 0..block.instructions.len() {
                if let MIRInstruction::BinaryOp {
                    dst,
                    left,
                    op,
                    right,
                ...
                } = &block.instructions[i]
                {
                    if let (
                        ...
                        MIRValue::Constant { value: r_val, .. },
                    ) = (left, right)
                    {
                        if l_ty.is_numeric() {
                            if let (Ok(l_num), Ok(r_num)) =
                                (l_val.parse::<i64>(), r_val.parse::<i64>())
                            {
                                let result = match op {
                                    ...
                                    crate::frontend::token::TokenType::Slash if r_num != 0 => {
                                        Some(l_num / r_num)
                                    }
                                    crate::frontend::token::TokenType::Modulo if r_num != 0 => {
                                        Some(l_num % r_num)
                                    }
                                    _ => None,
                                };
                            ...
                            } else if let (Ok(l_num), Ok(r_num)) =
                                (l_val.parse::<f64>(), r_val.parse::<f64>())
                            {
                                let result = match op {
                                    crate::frontend::token::TokenType::Plus => Some(l_num + r_num),
                                    crate::frontend::token::TokenType::Minus => Some(l_num - r_num),
                                    crate::frontend::token::TokenType::Star => Some(l_num * r_num),
                                    crate::frontend::token::TokenType::Slash if r_num != 0.0 => {
                                        Some(l_num / r_num)
                                    }
                                    _ => None,
                                };
            ...
            for instr in &block.instructions {
                match instr {
                    ...
                    | MIRInstruction::IndirectCall { args, .. } => {
                        ...
                        if let MIRInstruction::IndirectCall {
                            callee: MIRValue::Variable { name, .. },
                            ..
                        } = instr
                        {
                            used_vars.insert(name.clone());
                ...
                match &block.instructions[i] {
                    ...
                    | MIRInstruction::Cast { dst, .. } => {
                        if !used_vars.contains(dst)
                            && !func.variables.contains_key(dst)
                            && !func.params.contains(dst)
                        {
                    ...
                    MIRInstruction::Call { dst: _, .. }
                    | MIRInstruction::IndirectCall { dst: _, .. } => {
