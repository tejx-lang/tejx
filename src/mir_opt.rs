                    ...
                    if let (MIRValue::Constant { value: l_val, ty: l_ty }, MIRValue::Constant { value: r_val, .. }) = (left, right) {
                        if l_ty.is_numeric() {
                            if let (Ok(l_num), Ok(r_num)) = (l_val.parse::<i64>(), r_val.parse::<i64>()) {
                                let result = match op {
                                    crate::token::TokenType::Plus => Some(l_num + r_num),
                                    crate::token::TokenType::Minus => Some(l_num - r_num),
                                    crate::token::TokenType::Star => Some(l_num * r_num),
                                    crate::token::TokenType::Slash if r_num != 0 => Some(l_num / r_num),
                                    crate::token::TokenType::Modulo if r_num != 0 => Some(l_num % r_num),
                                    _ => None,
                                };
                                ...
                                let result = match op {
                                    ...
                                    crate::token::TokenType::Minus => Some(l_num - r_num),
                                    crate::token::TokenType::Star => Some(l_num * r_num),
                                    crate::token::TokenType::Slash if r_num != 0.0 => Some(l_num / r_num),
                                    _ => None,
                                };
