...
double bench_copy() {
  ...
  for (int i = 0; i < 5000; i++) {
    for (int j = 0; j < 100000; j++)
      dst[j] = src[j]; // Naive copy to match JS/TX
  }
  return dst[0];
}
...
double bench_fib() { return (double)fib(40); }
...
int solve_nqueens(int n, int row, int col, int diag1, int diag2) {
  ...
  while (available != 0) {
    ...
    available ^= pos;
    count += solve_nqueens(n, row + 1, col | pos, (diag1 | pos) << 1,
                           (diag2 | pos) >> 1);
  }
  return count;
}

double bench_nqueens() { return (double)solve_nqueens(14, 0, 0, 0, 0); }
...
double bench_ackermann() { return (double)ackermann(3, 10); }
...
void run(string name, double (*f)()) {
  ...
  cout << " -> [" << setprecision(2) << res << "] : " << fixed
       << setprecision(2) << duration<double, milli>(e - s).count() << " ms"
       << endl;
}
